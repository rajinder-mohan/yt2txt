...
def get_ytdlp_options(base_opts: dict = None) -> dict:
    """
    Get yt-dlp options with proxy support and rate limiting if configured.
    
    Priority for proxy:
    1. Stored proxy from database (ytdlp_proxy setting)
    2. YTDLP_PROXY environment variable
    ...
    # Add proxy support
    # First check database settings, then environment variable
    stored_proxy = get_setting("ytdlp_proxy")
    if stored_proxy and stored_proxy.strip():
        base_opts["proxy"] = stored_proxy.strip()
        print(f"Using proxy from database settings: {stored_proxy[:50]}...")
    else:
        # Fallback to environment variable
        proxy = os.getenv("YTDLP_PROXY")
        if proxy and proxy.strip():
            base_opts["proxy"] = proxy.strip()
            print(f"Using proxy from environment variable: {proxy[:50]}...")
...
async def get_setting_endpoint(setting_key: str, user: str = Depends(authenticate_api_request)):
    ...
    **Common Setting Keys:**
    ...
    - `ytdlp_proxy`: Proxy URL for yt-dlp requests (e.g., "http://user:pass@proxy.example.com:8080")
...
async def set_setting_endpoint(
    ...
    **Common Settings:**
    ...
    - `ytdlp_proxy`: Set proxy URL for yt-dlp (e.g., "http://user:pass@proxy.example.com:8080")
...
class ProxyTestRequest(BaseModel):
    proxy: str = Field(..., description="Proxy URL to test (format: 'http://user:pass@host:port')")


@app.post("/api/admin/test-proxy")
async def test_proxy_endpoint(
    ...
    **Test Proxy Configuration**
    ...
    Tests a proxy URL by attempting to extract info from a test video.
    
    **What it does:**
    - Tests proxy connectivity
    - Attempts to use proxy with yt-dlp
    - Returns test results
    
    **Input:**
    - `proxy`: Proxy URL to test (e.g., "http://username:password@proxy.example.com:8080")
    
    **Response:**
    - Returns success message if proxy works
    - Returns error message if proxy fails
    ...
    **Use cases:**
    ...
    - Debug proxy issues
    """
    if not request.proxy or not request.proxy.strip():
        raise HTTPException(
            status_code=400,
            detail="Proxy URL cannot be empty"
        )
    
    try:
        # Test with a simple video (short test video)
        test_video_id = "abc123xyz45"  # Dummy test video ID
        test_url = f"https://www.youtube.com/watch?v={test_video_id}"
        
        # Create yt-dlp options with proxy
        ydl_opts = {
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            # Try to extract video info (this will test proxy)
            info = ydl.extract_info(test_url, download=False)
            
            if info:
                return {
                    "message": f"Proxy test successful! Successfully connected through proxy.",
                    "video_title": info.get("title", "Unknown")
                }
            else:
                raise HTTPException(
                    status_code=500,
                    detail="Proxy test failed: Could not extract video info"
                )
    except Exception as e:
        error_msg = str(e)
        # Check for common proxy errors
        if "proxy" in error_msg.lower() or "connection" in error_msg.lower() or "unavailable" in error_msg.lower():
            raise HTTPException(
                status_code=400,
                detail=f"Proxy test failed: {error_msg}"
            )
        else:
            raise HTTPException(
                status_code=500,
